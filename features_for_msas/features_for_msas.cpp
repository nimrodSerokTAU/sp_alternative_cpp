...
int main()
{
    ...
    for (const auto& modelConfig : config["models_list"]) {
        int gapOpenCost = modelConfig["gap_open_cost"];
        int gapExtendCost = modelConfig["gap_extend_cost"];
        std::string substitutionMatrix = modelConfig["matrix_file_name"];
        EvoModel evoModel(gapOpenCost, gapExtendCost, substitutionMatrix);
        models.push_back(evoModel);
    }

    Configuration configuration(models,
                            static_cast<SopCalcTypes>(config["sop_calc_type"]),
                            config["input_files_dir_path"],
                            {}, // additional_weights
                            {}, // k_values
                            {StatsOutput::DISTANCE_LABELS, StatsOutput::RF_LABEL} // stats_output
	);
    std::cout << "SP Alternative (C++) - MSA Analysis Tool" << std::endl;
    std::cout << "Input directory: " << configuration.input_files_dir_path << std::endl;
